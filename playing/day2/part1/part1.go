...
func Process(items []string, gameLimit ColourCount) int {
	total := 0
	for _, item := range items {
		id, colours := ParseLine(item)
		if colours.red <= gameLimit.red && colours.green <= gameLimit.green && colours.blue <= gameLimit.blue {
			total += id
		}
	}
	return total
